...
class DomeController:
	...
	def createRoofMSG(self,status, sensor1, sensor2, sensor3, ubication):
		roof_msg = roof()
	...
	def action_open(self):
		...
		while msg != "FINISH":
			if self.roof_status.state != self.STATES[9]:
				roof_msg = self.createRoofMSG(msg,True,False,True,1)
	...
	def action_close(self):
		...
		while msg != "FINISH":
			if self.roof_status.state != self.STATES[8]:
				roof_msg = self.createRoofMSG(msg,False,True,False,1)
				self.pub.publish(roof_msg)
